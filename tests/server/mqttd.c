...
static curl_socket_t mqttit(curl_socket_t fd)
{
  ...
  if(rc > 6) {
    if(buffer[0] == MQTT_MSG_SUBSCRIBE) {
      ...
      remaining_length = decode_length(&buffer[1], rc-1, &bytes);
      if(remaining_length != (rc - bytes - 1)) {
        logmsg("Wrong remaining length, got %d expected %d",
               remaining_length, rc - bytes - 1);
        goto end;
      }
      i = 1 + bytes;
      /* two bytes packet id */
      packet_id = (unsigned short)((buffer[i] << 8) | buffer[i + 1]);
    ...
    else if((buffer[0]& 0xf0) == (MQTT_MSG_PUBLISH & 0xf0)) {
      size_t bytes;
      size_t remaininglength;
      size_t topiclen;
      logmsg("Incoming PUBLISH");
      remaininglength = decode_length(&buffer[1], rc-2, &bytes);
      if(remaininglength != (rc - bytes - 1)) {
        logmsg("wrong remaining length: %ld", (long)remaininglength);
        goto end;
      }
